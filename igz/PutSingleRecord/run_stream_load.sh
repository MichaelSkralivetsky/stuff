...
for shard in $SHARDS; do
	for gets in $GETS; do
		for size in $SIZES; do
			...
			./make_workload.sh -n $workload -g 0 -w $worker -r 0 -s 10.10.1.14 -c 12 -p /tmp/payload -d 20 -f $stream/0 -h Content-Type="\"application/json"\",X-igz-function="\"PutSingleRecord"\"

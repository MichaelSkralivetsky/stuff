...
for ((i=0;i<$num_files;i++)); do
	...
	#key=${keys[$i]}
	#key=$(($RANDOM + ($RANDOM % 2) * 32768))
	data=`dd if=/dev/urandom bs=1 count=$size|base64`
	echo -n "{\"attr\": {\"S\": \"test$i\"},\"attr1\": {\"B\": \"" > tmp2
	echo -n "$data"|sed 's/ //g'  >> tmp2
	echo -n "\"}}" >> tmp2
	cat tmp2 | tr -d '\n' > tmp3
	aws dynamodb put-item --table-name dynamodb --item file://tmp3
	echo -n "{"\"TableName"\": \"dynamodb\", \"ConsistentRead\": true, \"Key\": {\"attr\": {\"S\": "\"test$i"\"}}}" > $file
	#echo -n "$data"|sed 's/ //g'  >> $file
	#echo -n "\"}, \"attr\": {\"S\": \"test$i\"}}, "\"TableName"\": "\"dynamodb"\"}" >> $file

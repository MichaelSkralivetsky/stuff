...
if [ -z $random_files ]; then
	...
	for ((i=$gets;i<$load;i++)); do
		if [ -z $f ]; then
			...
			echo -e "\tAuthorization=\"AWS4-HMAC-SHA256 Credential=AKIAI6LU27UN3AIWNIQQ/$base_date/us-west-2/dynamodb/aws4_request,SignedHeaders=content-type;host;x-amz-date;x-amz-target,Signature=$signature"\" >> wl.tmp
			echo -e "\tx-amz-date=\"$full_date"\" >> wl.tmp
			echo -e "\tContent-Type=\"application/x-amz-json-1.0"\" >> wl.tmp
			echo -e "\tx-amz-target=\"DynamoDB_20120810.GetItem"\" >> wl.tmp
	done
else
	### "going random"
	#generate GET's
